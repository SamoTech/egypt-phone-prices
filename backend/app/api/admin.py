...
# Seed devices — top models per brand with realistic 2025/2026 EGP prices
SEED_DEVICES = [
    # Samsung
...
def _auth(x_admin_token: str = Header(default="")):
    if x_admin_token != settings.ADMIN_TOKEN:
        raise HTTPException(status_code=403, detail="Invalid admin token")


@router.post("/seed")
async def seed_data(db: AsyncSession = Depends(get_db), _: None = Depends(_auth)):
    """Seed brands + devices + prices from hardcoded data."""
    brand_ids = {}
    ...
    for d in SEED_DEVICES:
        ...
        for p in d.get("prices", []):
            ...
            await db.execute(
                text("""
                    INSERT INTO prices (device_id, retailer_id, price_egp, product_url)
                    VALUES (:device_id, :retailer_id, :price_egp, :product_url)
                """),
                {
                    "device_id": device_id,
                    "retailer_id": retailer_id,
                    "price_egp": p["price"],
                    "product_url": f"https://example.com/{d['slug']}",
                },
            )
            inserted_prices += 1
...
async def logs(db: AsyncSession = Depends(get_db), limit: int = 50):
    ...
    return [{"id": row.id, "retailer": row.retailer_slug, "status": row.status, "devices": row.devices_scraped, "error": row.error_message, "started": str(row.started_at)} for row in r.fetchall()]
